#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_STAMP="${PRIME_RUN_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_NAME="${OLMO_RUN_DIR_NAME:-prime_opd_full_vocab_ctx81920_g8_${RUN_STAMP}}"
RUN_ROOT="${PRIME_RUN_ROOT:-/alloc/prime-opd-runs}/${RUN_NAME}"

mkdir -p "${RUN_ROOT}"
printf '%s\n' "${RUN_ROOT}" > "${PRIME_RUN_ROOT:-/alloc/prime-opd-runs}/latest_run"

export CUDA_VERSION="${CUDA_VERSION:-12.8.1}"
export GLOBAL_RANK=0
export OLMO_RUN_DIR_NAME="${RUN_NAME}"
export PRIME_NODE_LAYOUT=1node
export PRIME_SINGLE_NODE_LABEL=0
export PRIME_TRAIN_PYTHON="${PRIME_TRAIN_PYTHON:-/opt/aimo-proof-pilot-venv/bin/python}"
export PRIME_TRAIN_ENTRYPOINT="${PRIME_TRAIN_ENTRYPOINT:-/opt/aimo-proof-pilot/src/train.py}"

export PRIME_OPD_MODEL_PATH="${PRIME_OPD_MODEL_PATH:-/alloc/models/opd-32b-deploy/opd-32b-deploy}"
export PRIME_OPD_TEACHER_MODEL_PATH="${PRIME_OPD_TEACHER_MODEL_PATH:-/alloc/models/dpsk-v4-flash}"
# vLLM creates Unix-domain sockets below VLLM_RPC_BASE_PATH. Keep this path
# short enough for Linux sockaddr_un (107 bytes), independent of the verbose
# run name used for durable logs and checkpoints.
export PRIME_3NODE_TMP_ROOT="${PRIME_3NODE_TMP_ROOT:-/alloc/pp/${RUN_STAMP}}"
export PRIME_3NODE_RENDEZVOUS_DIR="${PRIME_3NODE_RENDEZVOUS_DIR:-${RUN_ROOT}/rdzv}"
export PRIME_3NODE_RUNTIME_BASE="${PRIME_3NODE_RUNTIME_BASE:-${RUN_ROOT}/runtime}"
export PRIME_OPD_OUTPUT_ROOT="${PRIME_OPD_OUTPUT_ROOT:-${RUN_ROOT}/output}"
export PRIME_OPD_LOG_ROOT="${PRIME_OPD_LOG_ROOT:-${RUN_ROOT}/logs}"
export PRIME_OPD_CHECKPOINT_ROOT="${PRIME_OPD_CHECKPOINT_ROOT:-${RUN_ROOT}/checkpoints}"
export DG_JIT_CACHE_DIR="${DG_JIT_CACHE_DIR:-/alloc/prime-opd-smoke/cache/deep_gemm}"

# Raw CUDA 12.8 node: use its compatible vLLM 0.24 installation and the
# validated vLLM-bundled DeepGEMM fallback for Prime-RL FP8 training.
export PRIME_RL_RUNTIME_INSTALL_VLLM="${PRIME_RL_RUNTIME_INSTALL_VLLM:-0}"
export PRIME_RL_RUNTIME_VLLM_EXPECTED_VERSION="${PRIME_RL_RUNTIME_VLLM_EXPECTED_VERSION:-0.24.0}"

# One-node production placement: policy 0-3, trainer 4-5, teacher 6-7.
export PRIME_TRAIN_GPUS="${PRIME_TRAIN_GPUS:-2}"
export PRIME_VLLM_TP="${PRIME_VLLM_TP:-1}"
export PRIME_VLLM_DP="${PRIME_VLLM_DP:-4}"
export PRIME_VLLM_API_SERVER_COUNT="${PRIME_VLLM_API_SERVER_COUNT:-4}"
export PRIME_OPD_TEACHER_TP="${PRIME_OPD_TEACHER_TP:-2}"
export PRIME_OPD_TEACHER_DP="${PRIME_OPD_TEACHER_DP:-1}"
export PRIME_OPD_TEACHER_GPU_IDS="${PRIME_OPD_TEACHER_GPU_IDS:-6,7}"
# Keep explicit headroom for teacher hidden-state extraction. At TP=2 this
# requires a shorter teacher context than the policy rollout server.
export PRIME_OPD_TEACHER_GPU_MEMORY_UTILIZATION="${PRIME_OPD_TEACHER_GPU_MEMORY_UTILIZATION:-0.96}"

export MAX_TRAIN_STEPS="${MAX_TRAIN_STEPS:-1000}"
export PRIME_OPD_CTX_LEN="${PRIME_OPD_CTX_LEN:-81920}"
export PRIME_OPD_VLLM_MAX_MODEL_LEN="${PRIME_OPD_VLLM_MAX_MODEL_LEN:-90112}"
export PRIME_OPD_TEACHER_VLLM_MAX_MODEL_LEN="${PRIME_OPD_TEACHER_VLLM_MAX_MODEL_LEN:-90112}"
export PRIME_OPD_TEACHER_KV_CACHE_MEMORY_BYTES="${PRIME_OPD_TEACHER_KV_CACHE_MEMORY_BYTES:-39728447488}"
export PRIME_OPD_TEACHER_CPU_OFFLOAD_GB="${PRIME_OPD_TEACHER_CPU_OFFLOAD_GB:-16}"
export PRIME_OPD_COMPLETION_TOKENS="${PRIME_OPD_COMPLETION_TOKENS:-65000}"
export PRIME_OPD_EVAL_COMPLETION_TOKENS="${PRIME_OPD_EVAL_COMPLETION_TOKENS:-65000}"
export PRIME_OPD_BATCHED_TOKENS="${PRIME_OPD_BATCHED_TOKENS:-16384}"
export PRIME_OPD_TEACHER_BATCHED_TOKENS="${PRIME_OPD_TEACHER_BATCHED_TOKENS:-90112}"
export PRIME_OPD_POLICY_MAX_NUM_SEQS="${PRIME_OPD_POLICY_MAX_NUM_SEQS:-6}"
export PRIME_OPD_TEACHER_MAX_NUM_SEQS="${PRIME_OPD_TEACHER_MAX_NUM_SEQS:-1}"
export PRIME_OPD_MAX_INFLIGHT_ROLLOUTS="${PRIME_OPD_MAX_INFLIGHT_ROLLOUTS:-24}"
export PRIME_OPD_MAX_INFLIGHT_QUESTIONS="${PRIME_OPD_MAX_INFLIGHT_QUESTIONS:-4}"
export PRIME_VLLM_EXTRA="${PRIME_VLLM_EXTRA:-{\"kv_cache_dtype\":\"fp8\",\"block_size\":256,\"scheduling_policy\":\"priority\",\"compilation_config\":{\"pass_config\":{\"fuse_allreduce_rms\":false}}}}"

# Prime-RL forms one environment group from eight independently sampled proof
# candidates. Four stop after proof generation; the best four continue.
export PRIME_BATCH_SIZE="${PRIME_BATCH_SIZE:-2}"
export PRIME_GROUP_SIZE="${PRIME_GROUP_SIZE:-4}"
export PRIME_PROOF_CANDIDATE_CONTINUE_COUNT="${PRIME_PROOF_CANDIDATE_CONTINUE_COUNT:-2}"
export PRIME_PROOF_SELECTOR_TOP_K="${PRIME_PROOF_SELECTOR_TOP_K:-3}"
export PRIME_PROOF_NUM_VERIFIERS="${PRIME_PROOF_NUM_VERIFIERS:-4}"
export PRIME_PROOF_REFINE_ROUNDS="${PRIME_PROOF_REFINE_ROUNDS:-1}"
export PRIME_PROOF_REFINE_REVIEW_N="${PRIME_PROOF_REFINE_REVIEW_N:-2}"

# 4 x 81,920 = 327,680 packed tokens per optimizer step for this fast check.
export PRIME_PACKED_SEQUENCES_PER_STEP="${PRIME_PACKED_SEQUENCES_PER_STEP:-4}"
export PRIME_MAX_OFF_POLICY_STEPS="${PRIME_MAX_OFF_POLICY_STEPS:-24}"

export PRIME_OPD_EVAL_INTERVAL="${PRIME_OPD_EVAL_INTERVAL:-10}"
export PRIME_OPD_EVAL_NUM_EXAMPLES="${PRIME_OPD_EVAL_NUM_EXAMPLES:-8}"
export PRIME_OPD_EVAL_GROUP_SIZE="${PRIME_OPD_EVAL_GROUP_SIZE:-1}"
export PRIME_CHECKPOINT_INTERVAL="${PRIME_CHECKPOINT_INTERVAL:-100}"
export PRIME_CHECKPOINT_KEEP_LAST="${PRIME_CHECKPOINT_KEEP_LAST:-2}"
export PRIME_CHECKPOINT_WEIGHTS_ONLY="${PRIME_CHECKPOINT_WEIGHTS_ONLY:-true}"

export PRIME_TRAINER_FP8="${PRIME_TRAINER_FP8:-true}"
export PRIME_TRAINER_COMPILE="${PRIME_TRAINER_COMPILE:-true}"
export PRIME_VLLM_ENFORCE_EAGER=false
export PRIME_OPD_TEACHER_VLLM_ENFORCE_EAGER=false
export PRIME_TEMPERATURE="${PRIME_TEMPERATURE:-1.0}"
export WANDB_MODE=online
export WANDB_PROJECT="${WANDB_PROJECT:-olmo3-prime-rl-full-vocab}"

echo "[prime-opd-production] run_root=${RUN_ROOT} ctx=${PRIME_OPD_CTX_LEN} steps=${MAX_TRAIN_STEPS} group=${PRIME_GROUP_SIZE}"
exec bash "${SCRIPT_DIR}/prime_rl_opd_8node_full_vocab_dpsk_ctx81920_nodes345.sh"
