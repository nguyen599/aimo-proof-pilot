...
LOCAL_COMPILE_ROOT="${PRIME_SFT_LOCAL_COMPILE_ROOT:-/dev/shm/prime-sft-compile/${RUN_NAME}/node${NODE_LABEL}}"
mkdir -p "${RENDEZVOUS_DIR}" "${RUNTIME_BASE}" "${RUNTIME_DEPS_DIR}" \
  "${OUTPUT_ROOT}" "${LOG_ROOT}" "${CACHE_ROOT}" "${CHECKPOINT_ROOT}" \
  "${LOCAL_TMP_ROOT}" "${LOCAL_COMPILE_ROOT}"
...
export TRITON_CACHE_DIR="${LOCAL_COMPILE_ROOT}/triton"
export TORCHINDUCTOR_CACHE_DIR="${LOCAL_COMPILE_ROOT}/torchinductor"
export CUDA_CACHE_PATH="${LOCAL_COMPILE_ROOT}/cuda"
export PRIME_RL_ISOLATE_COMPILE_CACHE="${PRIME_RL_ISOLATE_COMPILE_CACHE:-1}"
export UV_CACHE_DIR="${LOCAL_TMP_ROOT}/uv"
mkdir -p "${TMPDIR}" "${XDG_CACHE_HOME}" "${PIP_CACHE_DIR}" \
  "${TRITON_CACHE_DIR}" "${TORCHINDUCTOR_CACHE_DIR}" "${CUDA_CACHE_PATH}" "${UV_CACHE_DIR}"
