...
"${PYTHON_BIN}" -m pip install --no-cache-dir debugpy expecttest
...
MAGI_ATTENTION_SKIP_CUDA_BUILD=1 \
    "${PYTHON_BIN}" -m pip install --no-build-isolation --no-deps "${MAGI_ATTENTION_DIR}"
"${PYTHON_BIN}" -m pip install --no-build-isolation --no-deps "${MAGI_ATTENTION_DIR}/extensions"
